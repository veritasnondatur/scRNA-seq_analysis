## scRNA-seq data analysis of midface E10.5 with Seurat
# author: Vera Laub
...
out_folder <- "/Users/veralaub/Documents/postdoc/bioinformatics/data/scRNA-seq/scRNA-seq_midface/MF-E10.5_ML10_071719/results/"
...
p <- RidgePlot(so_midface_E10.5, 
               ...
               ncol = 1, log = TRUE)
out_path <- paste(out_folder, "midface_E10.5.qc.unfiltered.pdf", sep = "")
...
p <- RidgePlot(so_midface_E10.5,
               features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), 
               ncol = 1, log = TRUE)
out_file <- paste(out_folder, "midface_E10.5.qc.filtered.pdf", sep = "")
...
p <- ElbowPlot(so_midface_E10.5, ndims = 100)
out_file <- paste(out_folder, "midface_E10.5.qc.ellbowplot.pdf", sep = "")
...
so_midface_E10.5 <- FindNeighbors(so_midface_E10.5, dims = 1:pca_dim_sel)         # "Cell clusters were identified by constructing a shared nearest neighbor graph followed by a modularity optimization-based clustering algorithm (Leiden algorithm) using the top 60 principal components as determined by PCA." (Losa et al., 2023)
so_midface_E10.5 <- FindClusters(so_midface_E10.5, resolution = 0.8)              # "Clustering was performed at multiple resolutions between 0.2 and 2, and optimal resolution was determined empirically based on the expression of known population markers (resolution = 0.8)." (Losa et al., 2023)

# Save Seurat object
saveRDS(so_midface_E10.5, file = "/Users/veralaub/Documents/postdoc/bioinformatics/data/scRNA-seq/scRNA-seq_midface/MF-E10.5_ML10_071719/results/so_midface_E10.5.rds")

# Read Seurat object (for post-hoc analysis)
so_midface_E10.5 <- readRDS(file = "/Users/veralaub/Documents/postdoc/bioinformatics/data/scRNA-seq/scRNA-seq_midface/MF-E10.5_ML10_071719/results/so_midface_E10.5.rds")

# Markers by cluster
out_file <- paste(out_folder, "/midface_E10.5.markers.txt", sep = "")
markers <- FindAllMarkers(so_midface_E10.5, 
                          min.pct = 0.1, 
...
p <- FeaturePlot(so_midface_E10.5, 
                 ...
                 reduction = "umap")
out_file <- paste(out_folder, "/midface_E10.5.UMAP.CellCycleRegressed.pdf", sep = "")
...
p <- DimPlot(object = so_midface_E10.5,
             ...
             label = TRUE)
out_file <- paste(out_folder, "/midface_E10.5.UMAP.clusters.pdf", sep = "")
pdf(out_file, width = 15, height = 10)
plot(p)
dev.off()
...
out_file <- paste(out_folder, "/midface_E10.5.UMAP.goi.pdf", sep = "")
pdf(out_file, width = 7, height = 5)
for (gene in goi) {
  p <- FeaturePlot(so_midface_E10.5, gene)
  plot(p)
}
dev.off()
...
out_file <- paste(out_folder, "/midface_E10.5.UMAP.goi.multipanel.pdf", sep = "")
pdf(out_file, width = 25, height = 20)
plot(p)
dev.off()

# Vioplot (by cluster) of goi
out_file <- paste(out_folder, "/midface_E10.5.vioplot.goi.pdf", sep = "")
pdf(out_file, width = 15, height = 10)
for (gene in goi) {
  p <- VlnPlot(so_midface_E10.5, 
               features = gene, 
               pt.size = 0, 
               group.by = "seurat_clusters") +
...
so_midface_E10.5 <- AddModuleScore(so_midface_E10.5, features = gene_set_1, name = "CoexpressionScore")

# Visualize the Coexpression score in UMAP
p <- FeaturePlot(so_midface_E10.5, features = "CoexpressionScore1", 
                 pt.size = 0.5) + 
  scale_color_gradient2(low = "red", mid = "white", high = "blue", 
                        midpoint = median(so_midface_E10.5$CoexpressionScore1)) +
  labs(title = "Co-expression in midface E10.5 (scRNA-seq)", 
       x = "UMAP 1", 
       y = "UMAP 2", 
       color = "Co-expression\n(Pbx1, Draxin)") +  # Custom legend title
  theme_minimal()
out_file <- paste(out_folder, "/midface_E10.5.UMAP.Pbx1+Draxin.co-expression.pdf", sep = "")
pdf(out_file, width = 7, height = 5)
plot(p)
dev.off()
...
Pbx1_expr <- FetchData(so_midface_E10.5, vars = "Pbx1")
Draxin_expr <- FetchData(so_midface_E10.5, vars = "Draxin")
...
so_midface_E10.5$MutualExclusive_Pbx1 <- mutually_exclusive_Pbx1
so_midface_E10.5$MutualExclusive_Draxin <- mutually_exclusive_Draxin
so_midface_E10.5$Both_Expressed <- both_expressed
...
out_file <- paste(out_folder, "/midface_E10.5.UMAP.mutual.exclusive.Pbx1_Draxin.pdf", sep = "")
...
out_file <- paste(out_folder, "/midface_E10.5.VennDiagram.Pbx1+Draxin.pdf", sep = "")
...
venn.plot <- venn.diagram(
  ...
  main = "Venn Diagram of Gene Expression, midface E10.5",  # Main title
...
# UMAP clusters
DimPlot(so_midface_E10.5, reduction = "umap",label = TRUE)

# Pbx1/2 and Draxin expression
plot <- FeaturePlot(so_midface_E10.5, 
                    features = c("Pbx1", "Pbx2", "Pbx3", "Draxin"),                    
                    cols = c('lightgray', 'blue'),
                    pt.size = 0.01)  # Adjust pt.size to your desired value

# Use patchwork to add a title to the whole plot grid
plot + plot_annotation(title = "scRNA-seq analysis of murine midface epithelium, E10.5")
...
# Use patchwork to add a title to the whole plot grid
plot + plot_annotation(title = "scRNA-seq of murine midface epithelium, E10.5: epithelial markers")
...
# Use patchwork to add a title to the whole plot grid
plot + plot_annotation(title = "scRNA-seq of murine midface epithelium, E10.5: mesenchymal markers")
...
# Add module scores to the Seurat object
data <- AddModuleScore(so_midface_E10.5, features = gene_set_1, name = "CoexpressionScore")
